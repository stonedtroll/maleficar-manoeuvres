...
export class TokenRepository {
    ...
    getPreviewToken(tokenId: string): Token | undefined {
        if (!this.isCanvasReady()) {
            return undefined;
        }

        const previewTokens = canvas!.tokens!.preview.children as globalThis.Token[];
        const previewToken = previewTokens.find(token => token.id === tokenId);

        return previewToken ? new Token(new TokenAdapter(previewToken)) : undefined;
    }
