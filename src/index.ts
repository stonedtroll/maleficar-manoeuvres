...
Hooks.once('init', async () => {
  try {
    ...
    ui.notifications?.error('Maleficar Manoeuvres failed to initialise');
  }
});
...
Hooks.once('ready', async () => {
  try {
    ...
    ui.notifications?.error('Maleficar Manoeuvres failed to start');
  }
});
...
async function startApplication(): Promise<void> {
  if (!moduleState.isInitialised || !moduleState.logger || !moduleState.container) {
    ...
    ui.notifications?.error('Maleficar Manoeuvres not properly initialised');
    return;
  }

  moduleState.logger.info('Game ready, setting up Maleficar Manoeuvres');
  ...
  try {
    ...
    moduleState.logger.info('Maleficar Manoeuvres setup complete');
    
  } catch (error) {
    moduleState.logger.error('Failed to initialise ManoeuvreApplication', error);
...
async function teardownModule(): Promise<void> {
  if (moduleState.logger) {
    moduleState.logger.info('Closing game, tearing down Maleficar Manoeuvres');
  }
  ...
  try {
    ...
    if (moduleState.logger) {
      moduleState.logger.info('Maleficar Manoeuvres teardown complete');
